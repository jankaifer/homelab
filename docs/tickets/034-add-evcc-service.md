...
- Assigned the Tesla Model 3 as the default vehicle on the commissioning loadpoint and enabled hourly vehicle SoC polling so disconnected placeholder charger state does not surface as 0%/0km vehicle data.
- Confirmed the direct Victron EVCS endpoint at `192.168.2.32:502` responds to EVCC's upstream `victron-evcs` template and switched the loadpoint from the disabled demo charger placeholder to the real EVCS charger. EOS Connect remains advisory/read-only and is not granted an EVCC command path.
