...
- Host networking enabled so the broker TLS hostname pinned to `127.0.0.1` on
  `frame1` also works from inside the container
...
- Production evaluation fails fast if `serialPort` is still `/dev/serial/by-id/usb-CHANGEME`
...
`frame1-vm` explicitly allows the placeholder serial path so local eval/build checks still work without hardware.
